const About = () => {
    return (
        <section className="section">
            <h2>Acerca de mi</h2>
            <p>
                Soy una persona responsable, me adapto a las tareas y necesidades que se requieran, se trabajar 
                en equipo, honesto, respetuoso, dedicado y siempre dispuesto a aprender cosas nuevas.
            </p>
            <p>
                Tengo conocimientos en el manejo del desarrollo web y en su diseño grafico, en la creacion y desarrollo 
                de proyectos en Django, tengo conociminetos basicos sobre el manejo de una base de datos, el uso de
                python y javascript.
            </p>
        </section>
    );
};

export default About;